...
class Inversion:
    ...
    def __init__(self, config, forward):
        """Specify retrieval subwindows for calculating measurement cost distributions."""
    ...
    def calc_prior(self, x, geom):
        """Calculate prior distribution of radiance.

        Depends on the location in the state space. Return the inverse covariance
        and its square root (for non-quadratic error residual calculation).
        """
    ...
    def calc_posterior(self, x, geom, meas):
        """Calculate posterior distribution of state vector.

        Depends on location in state space and radiance (via noise).
        """
    ...
    def calc_Seps(self, x, meas, geom):
        ...
        Depends on the location in the state space. This distribution is 
    ...
    def invert(self, meas, geom):
        ...
        TODO: remove nested function jac, err
        """
        ...
        _, Seps_inv_sqrt = self.calc_Seps(x0, meas, geom)

        def jac(x):
            """Calculate measurement Jacobian and prior Jacobians w.r.t. cost function.

            This is the derivative of cost with respect to the state, commonly
            known as the gradient or loss surface. The cost is expressed as a
            vector of 'residuals' with respect to the prior and measurement,
            expressed in absolute (not quadratic) terms for the solver; It is
            the square root of the Rodgers (2000) Chi-square version. All measurement
            ...
            _, _, _, Sa_inv_sqrt = self.calc_prior(x, geom)
        ...
        def err(x):
            """Calculate cost function in absolute (not quadratic) terms for solver.

            i.e., the square root of the  Rodgers (2000) Chi-square version.
            We concatenate 'residuals' due to measurment and prior terms,
            suitably scaled. All measurement distributions are calculated over
            subwindows of the full spectrum.
            """
            ...
            # Prior cost term
            xa, _, _, Sa_inv_sqrt = self.calc_prior(x, geom)
