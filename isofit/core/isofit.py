...
class Isofit:
    ...
    def _run_single_spectrum(self, index_start, index_stop):
        self._init_nonpicklable_objects()
    ...
    def run(self, profile=False):
        ...
        if profile:
            ...
            if n_cores == 1:
                self._run_single_spectrum(index_sets[0], index_sets[-1])
            else:
                for l in range(len(index_sets)-1):
                    results.append(pool.apply_async(self._run_single_spectrum, args=(index_sets[l],index_sets[l+1])))
