...
from isofit.data.download import release_metadata
...
def check_version():
    ...
    """
    metadata = release_metadata("isofit", "isofit", tag="latest")
    current = Version(__version__)
    latest = Version(metadata["tag_name"])

    if current <= latest:
        print(
            f"[x] Current version is less than or equal to latest: {current} <= {latest}"
        )
    else:
        print(f"[OK] Current version is greater than latest: {current} > {latest}")
