...
def filter_image(out, thresh=100):
    masks = []
    for i in np.unique(out)[::-1]:
        shift = out - i
        shift[shift < 0] = 0
        label, n = ndimage.label(shift)
        sizes = ndimage.sum(shift, label, range(n + 1))
        mask = sizes > thresh
        masks.append(mask[label])

    for i, mask in enumerate(masks):
        if not i:
            final = mask.astype(int)
        else:
            final += mask.astype(int)
...
def multicomponent_classification(
    ...
    # Get wavelength
    wavelength_file = "/Users/bgreenbe/Data/BIOSCAPE/ang_wavelengths.txt"
    if wavelength_file:
    ...
    if clean:
        ...
        out_filter = filter_image(out, thresh)
        out[...] = out_filter
