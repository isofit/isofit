...
def define_surface_types(
    ...
    for line in range(len(classes)):
        ...
        total = (
            np.array(rho[b450] > tsip["cloud"]["toa_threshold_values"][0], dtype=int)
            + np.array(rho[b1250] > tsip["cloud"]["toa_threshold_values"][1], dtype=int)
            + np.array(rho[b1650] > tsip["cloud"]["toa_threshold_values"][2], dtype=int)
        )
...
def get_metadata_from_loc(
    ...
    if "Easting (m)" or "Northing (m)" in loc_dataset.GetMetadata().values():
        ...
        lat, lon = utm.to_latlon(
            loc_data[0, valid].flatten(),
            loc_data[1, valid].flatten(),
            zone_number=zone,
