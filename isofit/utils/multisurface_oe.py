...
from isofit.utils import segment, extractions, empirical_line, analytical_line, surface_model
...
def main(rawargs=None):
    ...
    for infile_name in ['input_loc', 'input_obs']:
        input_dataset = envi.open(envi_header(infiles[infile_name]))
    ...
    if opt["sensor"] == 'ang':
        ...
        if args.datetime is None:
            raise ValueError('With sensor NA-, --datetime argument is required')
    ...
    if args.wavelength_path:
        ...
        wl = np.array([float(x) for x in rdn_dataset.metadata["wavelength"]])
        fwhm = np.array([float(x) for x in rdn_dataset.metadata["fwhm"]])
        del rdn_dataset
