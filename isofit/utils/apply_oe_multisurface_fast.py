...
def main(rawargs=None):
    ...
    if args.presolve == 1:
        ...
        if not exists(envi_header(paths.h2o_subs_path)) or not exists(paths.h2o_subs_path):
            ...
            build_presolve_config(paths, h2o_grid, args.n_cores, True, args.surface_category,
    ...
    if not exists(paths.state_subs_path) or \
        ...
        for st in surface_types:
            build_main_config(paths, lut_params, h2o_lut_grid, elevation_lut_grid, to_sensor_azimuth_lut_grid,
                          ...
                          True, args.n_cores, args.surface_category,
...
class Pathnames():
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        self.class_subs_path = abspath(join(self.output_directory, self.fid + '_subs_classes'))
    ...
    def add_surface_subs_files(self, surface_type):
        ...
        self.surface_config_paths[surface_type] = abspath(join(self.config_directory, f'{self.fid}_{surface_type}_isofit.json'))
...
def calc_modtran_max_water(paths: Pathnames):
...
def build_main_config(paths: Pathnames, lut_params: LUTConfig, h2o_lut_grid: np.array = None,
    ...
    if use_emp_line:
        if surface_type is None:
            ...
            isofit_config_modtran['output']['posterior_uncertainty_file'] = paths.surface_subs_files[surface_type]['uncert']
            isofit_config_modtran['output']['estimated_reflectance_file'] = paths.surface_subs_files[surface_type]['rfl']
