...
class LibRadTranRT(TabularRT):
    ...
    def rebuild_cmd(self, point, fn):
        ...
        with open(infilepath0, "r") as fin:
            lat, lon, yr, mon, day, hour, mn = None, None, None, None, None, None, None
            for line in fin.readlines():
                ...
                if "latitude N" in line:
                    ...
                    yr, mon, day, hour, mn, sec = [float(q) for q in line.split()[1:]]
        ...
        with open(scriptfilepath, "w") as f:
            ...
            f.write(
                "(cd %s/bin && ./zenith %s -a %s -o %s -y %s %s %s %s %s > %s)\n"
                % (
                    self.libradtran_dir,
                    "-s 0 -q",
                    lat,
