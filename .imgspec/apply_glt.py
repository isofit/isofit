...
def main():
    parser = argparse.ArgumentParser(
        description="Integrate multiple GLTs with a mosaicing rule"
    )
    parser.add_argument(
        "rawspace_file",
    ...
    if args.log_file is None:
        ...
        logging.basicConfig(
            format="%(message)s", level=args.log_level, filename=args.log_file
        )
    ...
    if args.mosaic:
        rawspace_files = np.squeeze(
            np.array(pd.read_csv(args.rawspace_file, header=None))
        )
    else:
    ...
    outmeta["coordinate system string"] = glt_dataset.metadata[
        "coordinate system string"
    ]
    if "band names" in first_file_dataset.metadata.keys():
        outmeta["band names"] = [
            first_file_dataset.metadata["band names"][i] for i in output_bands
        ]
    if "wavelength" in first_file_dataset.metadata.keys():
        outmeta["wavelength"] = [
            first_file_dataset.metadata["wavelength"][i] for i in output_bands
        ]
    ...
    logging.info(
        "GLT application complete, output available at: {}".format(args.output_filename)
    )
...
def apply_mosaic_glt_line(
    glt_filename: str,
    output_filename: str,
    rawspace_files: List,
    output_bands: np.array,
    line_index: int,
    args: List,
):
    """
    ...
    if line_index % 100 == 0:
        logging.info(
            "Beginning application of line {}/{}".format(line_index, glt.shape[0])
        )
        ...
        if os.path.isfile(rawspace_files[_idx]):
            ...
            if np.sum(linematch) > 0:
                output_dat[linematch, :] = rawspace_dat[
                    glt_line[linematch, 1][:, None],
                    glt_line[linematch, 0][:, None],
                    output_bands[None, :],
                ].copy()

    _write_bil_chunk(
        np.transpose(output_dat),
        output_filename,
        line_index,
